    ...
    <section class="JOH-Flex Rows-CC" style="padding:2em;">
      ...
      <dd class="JOH-Font-S" style="margin-bottom:1em;">
        ...
        ?>
          ...
          @case (1)
            <select id="{!! $id !!}" data-id="{!! $id !!}" data-attr_def_group_id="{{ $attr_def->attr_def_group_id }}" data-attr_def_id="{{ $attr_def->id }}" class="JOH-Font-S JOH-Border-1 JOH-Color-Form-Text" style="padding:0.5em;{!! $input_width !!}" placeholder="{{ $attr_def->name }}"{!! $required !!}>
          ...
          @break;
          @case (4)
            <select size="{!! ($attr_def->input_height ?: count($list)) !!}" id="{!! $id !!}" data-id="{!! $id !!}" data-attr_def_group_id="{{ $attr_def->attr_def_group_id }}" data-attr_def_id="{{ $attr_def->id }}" class="JOH-Font-S JOH-Border-1 JOH-Color-Form-Text" style="padding:0.5em;{!! $input_width !!}" placeholder="{{ $attr_def->name }}"{!! $required !!}>
            @foreach ($list as $item)
              <option value="{!! $item->value !!}"{!! ($item->value==$attr_def->default_attr_value ? ' selected' : '') !!}>{!! $item->name !!}</option>
            @endforeach
            </select>
          @break;
          @case (5)
            <select size="{!! ($attr_def->input_height ?: count($list)) !!}" multiple id="{!! $id !!}" data-id="{!! $id !!}" data-attr_def_group_id="{{ $attr_def->attr_def_group_id }}" data-attr_def_id="{{ $attr_def->id }}" class="JOH-Font-S JOH-Border-1 JOH-Color-Form-Text" style="padding:0.5em;{!! $input_width !!}" placeholder="{{ $attr_def->name }}"{!! $required !!}>
            @foreach ($list as $item)
